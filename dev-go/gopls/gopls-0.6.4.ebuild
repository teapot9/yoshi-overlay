...
EGO_SUM=(
	...
	"golang.org/x/tools v0.1.1-0.20210119222907-0a1a9685734a"
	"golang.org/x/tools v0.1.1-0.20210119222907-0a1a9685734a/go.mod"
...
S="${WORKDIR}/tools-${EGIT_COMMIT/\//-}/${PN}"

src_prepare() {
	sed -i -e 's|\(TestMultipleModules_Warning\)|_\1|g' \
		internal/regtest/diagnostics_test.go || die
	sed -i -e 's|\(TestInconsistentVendoring\)|_\1|g' \
		internal/regtest/vendor_test.go || die
	default
}
...
src_test() {
	go test -work ./... || die "go test failed"
}
